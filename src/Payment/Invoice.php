...
/**
 * Class Mobilpay\Payment\Invoice
 * @copyright NETOPIA
...
class Invoice
{
	...
	protected function loadFromXml(DOMNode $elem)
	{
		...
		if($attr == null)
		{
			throw new Exception('Mobilpay\Payment\Invoice::loadFromXml failed; currency attribute missing', self::ERROR_LOAD_FROM_XML_CURRENCY_ATTR_MISSING);
		}
		...
		if($elems->length == 1)
		{
			...
			if($elems->length == 1)
			{
				$this->billingAddress = new Mobilpay\Payment\Address($elems->item(0));
			}

			$elems = $addrElem->getElementsByTagName('shipping');
			if($elems->length == 1)
			{
				$this->shippingAddress = new Mobilpay\Payment\Address($elems->item(0));
			}
		}
		...
		if($elems->length == 1)
		{
			...
			if($elems->length > 0)
			{
				...
				foreach ($elems as $itemElem)
				{
					try
					{
						$objItem = new Mobilpay\Payment\Invoice\Item($itemElem);
						$this->items[] = $objItem;
		...
		if($elems->length == 1)
		{
			...
			foreach ($elems as $rateElem)
			{
				try
				{
					$objRate = new Mobilpay\Payment\Exchange\Rate($rateElem);
	...
	public function createXmlElement(DOMDocument $xmlDoc)
	{
		...
		if(($this->billingAddress instanceof Mobilpay\Payment\Address) || ($this->shippingAddress instanceof Mobilpay\Payment\Address))
		{
			$xmlAddr = null;
			if($this->billingAddress instanceof Mobilpay\Payment\Address)
			{
				try
				{
			...
			if($this->shippingAddress instanceof Mobilpay\Payment\Address)
			{
				try
				{
		...
		if(is_array($this->items) && sizeof($this->items) > 0)
		{
			...
			{
				if(!($item instanceof Mobilpay\Payment\Invoice\Item))
				{
					continue;
				}
				try
		...
		if(is_array($this->exchangeRates) && sizeof($this->exchangeRates) > 0)
		{
			...
			{
				if(!($rate instanceof Mobilpay\Payment\Exchange\Rate))
				{
					continue;
				}
				try
	...
	public function setBillingAddress(Mobilpay\Payment\Address $address)
	{
	...
	public function setShippingAddress(Mobilpay\Payment\Address $address)
	{
	...
	public function addHeadItem(Mobilpay\Payment\Invoice\Item $item)
	{
	...
	public function addTailItem(Mobilpay\Payment\Invoice\Item $item)
	{
	...
	public function addHeadExchangeRate(Mobilpay\Payment\Exchange\Rate $rate)
	{
	...
	public function addTailExchangeRate(Mobilpay\Payment\Exchange\Rate $rate)
	{
