...
/**
 * Class Mobilpay\Payment\Invoice\Item
 * @copyright NETOPIA
...
class Item
{
	...
	protected function loadFromXml(DOMNode $elem)
	{
		...
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid code element.', self::ERROR_LOAD_FROM_XML_CODE_ELEM_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid name element.', self::ERROR_LOAD_FROM_XML_NAME_ELEM_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid quantity element.', self::ERROR_LOAD_FROM_XML_QUANTITY_ELEM_MISSING);
		}
		$this->quantity = doubleval(urldecode($elems->item(0)->nodeValue));
		if($this->quantity == 0)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid quantity value=' . $this->quantity, self::ERROR_LOAD_FROM_XML_QUANTITY_ELEM_EMPTY);
		}

		$elems = $elem->getElementsByTagName('price');
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid price element.', self::ERROR_LOAD_FROM_XML_PRICE_ELEM_MISSING);
		}
		$this->price = doubleval(urldecode($elems->item(0)->nodeValue));
		if($this->price == 0)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid price value=' . $this->price, self::ERROR_LOAD_FROM_XML_PRICE_ELEM_EMPTY);
		}

		$elems = $elem->getElementsByTagName('vat');
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Invoice\Item::loadFromXml failed! Invalid vat element.', self::ERROR_LOAD_FROM_XML_VAT_ELEM_MISSING);
		}
