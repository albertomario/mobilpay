 ...
 * mobilPay
 *
 * @package   Mobilpa\Payment\Request\Notify
...
class Notify {
     ...
     * @param DOMNode $elem
     * @return Mobilpay\Payment\Request\Notify
    ...
    public function loadFromXml(DOMElement $elem) {
        ...
        if ($attr == null) {
            throw new Exception('Mobilpay\Payment\Request\Notify::loadFromXml failed; mandatory crc attribute missing', self::ERROR_LOAD_FROM_XML_CRC_ATTR_MISSING);
        }
        $this->_crc = $attr->nodeValue;
        $elems = $elem->getElementsByTagName('action');
        if ($elems->length != 1) {
            throw new Exception('Mobilpay\Payment\Request\Notify::loadFromXml failed; mandatory action attribute missing', self::ERROR_LOAD_FROM_XML_ACTION_ELEM_MISSING);
        }
        $this->action = $elems->item(0)->nodeValue;
        $elems = $elem->getElementsByTagName('customer');
        if ($elems->length == 1) {
            $this->customer = new Mobilpay\Payment\Address($elems->item(0));
        }
    ...
    public function createXmlElement(DOMDocument $xmlDoc) {
        ...
        if ($this->customer instanceof Mobilpay\Payment\Address) {
