...
/**
 * Class Mobilpay\Payment\Request\AbstractRequest
...
abstract class abstract
{
	 ...
	 * Optional for Mobipay\Payment\Request\Card 
	 */
	public $service		= null;
	...
	static public function factory($data)
	{
		...
		if(@$xmlDoc->loadXML($data) === true)
		{
			...
			$objPmReq = Mobilpay\Payment\Request\AbstractRequest::_factoryFromXml($xmlDoc);
			$objPmReq->_setRequestInfo(self::VERSION_XML, $data);
		}
		else
		{
			//try to create payment request from query string
			$objPmReq = Mobilpay\Payment\Request\AbstractRequest::_factoryFromQueryString($data);
	...
	static public function factoryFromEncrypted($envKey, $encData, $privateKeyFilePath, $privateKeyPassword = null)
	{
		...
		return Mobilpay\Payment\Request\AbstractRequest::factory($data);
	}
	...
	static protected function _factoryFromXml(DOMDocument $xmlDoc)
	{
		...
		if($elems->length != 1)
		{
			throw new Exception('factoryFromXml order element not found', Mobilpay\Payment\Request\AbstractRequest::ERROR_FACTORY_BY_XML_ORDER_ELEM_NOT_FOUND);
		}
		...
		{
			throw new Exception('factoryFromXml invalid payment request type=' . $attr->nodeValue, Mobilpay\Payment\Request\AbstractRequest::ERROR_FACTORY_BY_XML_ORDER_TYPE_ATTR_NOT_FOUND);
		}
		switch ($attr->nodeValue)
		{
		case Mobilpay\Payment\Request\AbstractRequest::PAYMENT_TYPE_CARD:
			$objPmReq = new Mobilpay\Payment\Request\Card();
			break;
		case Mobilpay\Payment\Request\AbstractRequest::PAYMENT_TYPE_SMS:
			$objPmReq =  new Mobilpay\Payment\Request\Sms();
			break;
		default:
			throw new Exception('factoryFromXml invalid payment request type=' . $attr->nodeValue, Mobilpay\Payment\Request\AbstractRequest::ERROR_FACTORY_BY_XML_INVALID_TYPE);
			break;
		}
	...
	protected function _parseFromXml(DOMNode $elem)
	{
		...
		{
			throw new Exception('Mobilpay\Payment\Request\Sms::_parseFromXml failed: empty order id', self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Request\Sms::loadFromXml failed: signature is missing', self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
		}
		...
		if($elems->length == 1)
		{
			$this->objPmNotify = new Mobilpay\Payment\Request\Notify();
