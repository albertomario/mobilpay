...
/**
 * Class Mobilpay\Payment\Request\Sms
...
class Sms extends AbstractRequest  
{
	...
	protected function _loadFromXml(DOMElement $elem)
	{
		...
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Request\Sms::loadFromXml failed: service is missing', self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
		}
	...
	protected function _loadFromQueryString($queryString)
	{
        ...
        {
        	throw new Exception('Mobilpay\Payment\Request\Sms::loadFromQueryString failed: signature is missing', self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
        }
        $this->signature = $reqParams['signature'];
        if(!isset($reqParams['service']))
        {
        	throw new Exception('Mobilpay\Payment\Request\Sms::loadFromQueryString failed: service is missing', self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
        }
        $this->service = $reqParams['service'];
        if(!isset($reqParams['tran_id']))
        {
        	throw new Exception('Mobilpay\Payment\Request\Sms::loadFromQueryString failed: empty order id', self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
        }
