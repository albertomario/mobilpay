...
/**
 * Class Mobilpay\Payment\Request\Card
...
class Card extends AbstractRequest  
{
	...
	protected function _loadFromXml(DOMElement $elem)
	{
		...
		if($elems->length != 1)
		{
			throw new Exception('Mobilpay\Payment\Reqeust\Card::loadFromXml failed; invoice element is missing', self::ERROR_LOAD_FROM_XML_ORDER_INVOICE_ELEM_MISSING);
		}
		
		$this->invoice = new Mobilpay\Payment\Invoice($elems->item(0));
	...
	{
		if(is_null($this->signature) || is_null($this->orderId) || !($this->invoice instanceof Mobilpay\Payment\Invoice))
		{
